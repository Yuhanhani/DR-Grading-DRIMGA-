...
vote_matrix = numpy.zeros((length, 3))
prediction_list = []
...
for i in range(len(model_list)):
    ...
    for row in range(length):
        index = numpy.argmax(prediction[row, :])
        vote_matrix[row, index] = vote_matrix[row, index] + 1
...
print(vote_matrix)
final_prediction = numpy.argmax(vote_matrix, axis=1)
final_pred_prob = numpy.zeros((length, 3))

for row in range(53):
    for i in range(len(model_list)):
        if numpy.argmax((prediction_list[i])[row]) == final_prediction[row]:
            final_pred_prob[row, :] = final_pred_prob[row, :] + (prediction_list[i])[row]

    final_pred_prob[row, :] = final_pred_prob[row, :]/numpy.amax(vote_matrix[row, :])
