...
transform_1 = data_augmentation2.data_augmentation_transform(phase='train')
...
# model = models.efficientnet_b3(weights=None, progress=False)
# model.load_state_dict(torch.load('/well/papiez/users/pea322/weights/efficientnet_b3_rwightman-cf984f9c.pth'))
# model.classifier[1] = nn.Linear(in_features=1536, out_features=6)  # 1280, 2560, 1536

# model = models.efficientnet_b7(weights='IMAGENET1K_V1', progress=False)
# model.classifier[1] = nn.Linear(in_features=2560, out_features=6)  # 2560

# model = models.efficientnet_v2_m(weights=True, progress=False)
# # model.load_state_dict(torch.load('/well/papiez/users/pea322/weights/efficientnet_v2_m-dc08266a.pth'))
...
# model = models.vit_b_16(weights=None, progress=False) # 1000 output
# model.load_state_dict(torch.load('/well/papiez/users/pea322/weights/vit_b_16-c867db91.pth'))
# vit doesn't converge, keep at around 20

# shuffle net, run time error due to exploding gradient.

# model = models.alexnet(weights=None, progress=True)
# model.load_state_dict(torch.load('/well/papiez/users/pea322/weights/alexnet-owt-4df8aa71.pth'))
# model.classifier[6] = nn.Linear(4096, 6)
# loss goes to infinity even with 0.0001 lr
